...
describe('number', () => {
  test.each([['number'], ['percent'], ['currency'], ['rating'], ['duration']] as const)('%s', (airtableType) => {
  ...
  test.each([['count'], ['autoNumber']] as const)('%s', (airtableType) => {
    const mapperPair = fieldMappers.number?.[airtableType]
    if (!mapperPair) {
      throw new Error(`Expected mapper pair for [number, ${airtableType}]`)
    }
...
describe('(number | null)[]', () => {
  test('multipleLookupValues', () => {
    ...
    expect(mapperPair.fromAirtable([1, 2, null])).toEqual([1, 2, null])
    expect(mapperPair.fromAirtable(null)).toEqual([])
    expect(mapperPair.fromAirtable(undefined)).toEqual([])
    expect(mapperPair.fromAirtable([])).toEqual([])
    expect(() => mapperPair.fromAirtable(['one'])).toThrow()

    expect(() => mapperPair.toAirtable([1, 2, null])).toThrow('read-only')
  })
})
